...
public class ConwayLocation extends Location {
    ...
    public void tick() {
        ...
        if (read) {
            ...
            boolean aliveHere = getGround().hasStatus(Alive.class);
        ...
        if (!read) { // Only tick the ground once per full Conway cycle
            super.tick();
        }
    }
    ...
    private int aliveNeighboursCount() {
        return (int) getExits().stream().map(exit -> exit.getDestination().getGround())
                .filter(ground -> ground.hasStatus(Alive.class)).count();
    }
